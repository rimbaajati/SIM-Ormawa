<template>
  <div
    :class="['liquid-ether-container', className]"
    :style="style"
    ref="mountRef"
  ></div>
</template>
...
const props = defineProps({
  ...
  colors: { type: Array, default: () => ["#CC561E", "#FF6D1F", "#FF9013"] },
  ...
  autoResumeDelay: { type: Number, default: 3000 },
  autoRampDuration: { type: Number, default: 0.6 },
});

// --- STATE MANAGEMENT (useRef -> ref) ---
...
// --- SETUP THREE.JS AND SIMULATION LOGIC ---

const paletteTex = computed(() => {
  return makePaletteTexture(props.colors);
});
...
// Subclasses of ShaderPass (Advection, ExternalForce, Viscous, Divergence, Poisson, Pressure)
...
class Output {
  ...
  init() {
    ...
    // RawShaderMaterial yang asli (dengan perbaikan sintaks)
    const outputMaterial = new THREE.RawShaderMaterial({
      ...
      uniforms: {
        ...
        palette: { value: paletteTex.value }, // Menggunakan computed ref
        bgColor: { value: bgVec4 },
      },
    });

    this.output = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), outputMaterial);
...
class WebGLManager {
  constructor(props) {
    ...
    this._onVisibility = () => {
      ...
      if (hidden) {
        ...
        // Menggunakan isVisibleRef.value
        this.start();
      }
    };
  ...
  loop() {
    ...
    this.render();
    rafRef.value = requestAnimationFrame(this._loop); // Menggunakan rafRef.value
  }
  start() {
    if (this.running) return;
    this.running = true;
...
onMounted(() => {
  ...
  const io = new IntersectionObserver(
    (entries) => {
      ...
      isVisibleRef.value = isVisible; // Menggunakan .value
      if (!webglRef.value) return;
  ...
  const ro = new ResizeObserver(() => {
    if (!webglRef.value) return;
    if (resizeRafRef.value) cancelAnimationFrame(resizeRafRef.value); // Menggunakan .value
...
// Watcher untuk mengaplikasikan perubahan props secara dinamis (menggantikan useEffect kedua di React)
watch(
  () => props,
  () => {
  ...
  { deep: true } // Mengamati perubahan pada objek props
);
...
<style scoped>
/* Anda mungkin perlu menyalin CSS dari LiquidEther.css ke sini atau mengimpornya secara global */
.liquid-ether-container {
  /* Gaya dasar jika tidak ada gaya eksternal yang diterapkan */
  width: 100%;
  height: 100%;
}
</style>